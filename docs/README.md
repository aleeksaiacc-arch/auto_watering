...
Система на базе: **LGT8F328P** (основной контроллер, управляет логикой полива и опросом датчиков); **резистивного датчика влажности** (измеряет сопротивление почвы — чем выше влажность, тем ниже сопротивление); **насоса** (подаёт воду в грунт, управляется через реле или транзистор); **ESP8266 ESP-01** (модуль Wi-Fi, связь с MQTT-брокером для мониторинга и удалённого управления).
